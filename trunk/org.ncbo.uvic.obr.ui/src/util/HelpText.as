package util
{
	...
	public class HelpText extends HBox 
	{
        ...
        public static const LINK_COLOR:uint = 0x234979;
		...
		public function set htmlText(html:String):void {
			if (html != textComponent.htmlText) {
				html = StringUtils.addLinkStyle(html, LINK_COLOR);
				textComponent.htmlText = html;
				dispatchEvent(new Event("htmlTextChanged"));
			}
		}
		
		public function get lastTextMouseDownEvent():MouseEvent {
		...
		public function get textComponent():Text {
			if (_text == null) {
				...
				_text.addEventListener(TextEvent.LINK, linkClicked);
				_text.addEventListener(MouseEvent.MOUSE_DOWN, textMouseDown);
			}
			return _text;
		}
		...
		private function moveHighlight(bounds:Rectangle, cornerRadius:uint = 0):void {
			...
			if (animateHighlightBox && (animateDuration > 0)) {
				var move:Move = new Move(highlightBox);
				move.xTo = bounds.x - 2;
				move.yTo = bounds.y - 2;
				move.duration = animateDuration;
				parallel.addChild(move);
				var size:Resize = new Resize(highlightBox);
				size.widthTo = bounds.width + 4;
				size.heightTo = bounds.height + 4;
				size.duration = animateDuration;
				parallel.addChild(size);
			} else {
				// no animation
